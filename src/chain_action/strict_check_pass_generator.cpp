...
StrictCheckPassGenerator::updatePasser( const WorldModel & wm )
{
    ...
    if ( M_passer->unum() != wm.self().unum() )
    {
        if ( M_first_point.dist2( wm.self().pos() ) > std::pow( 50.0, 2 ) )
        {
...
StrictCheckPassGenerator::updateReceivers( const WorldModel & wm )
{
          ...
          ++p )
    {
        ...
        if ( is_self_passer )
        {
            ...
            if(logtest)std::cout<<wm.time().cycle()<<"r"<<(*p)->unum();
            // if ( (*p)->unumCount() > 10 ) continue;
            if ( (*p)->posCount() > 10 ){
            	if(logtest)std::cout<<"for psc"<<std::endl;
            	continue;
            }
            if ( (*p)->isTackling() ){
            	if(logtest)std::cout<<"for tkl"<<std::endl;
            	continue;
            }
            if ( (*p)->pos().x > wm.offsideLineX()-0.2 )
            {
                dlog.addText( Logger::PASS,
                              ...
                              (*p)->pos().x, (*p)->pos().y,
                              wm.offsideLineX() );
                if(logtest)std::cout<<"for ofs"<<std::endl;
                continue;
            }
            ...
            {
            	if(logtest)std::cout<<"for gol"<<std::endl;
                continue;
            }
        }
        else
...
//                continue;
        ...
        if ( (*p)->pos().dist2( M_first_point ) > max_dist2 ){
        	if(logtest)std::cout<<"for dist"<<std::endl;
        	continue;
        }
        if(logtest)std::cout<<" is ok"<<std::endl;
                                             ...
                                             const Receiver & receiver )
{
	...
	if(M_passer->unum()!=wm.self().unum())return;
	if(logtest)cout<<"******"<<wm.time().cycle()<<"t"<<receiver.player_->unum()<<"********"<<endl;
    ...
    int ANGLE_DIVS = 18;//8;//24;
    double ANGLE_STEP = 360.0 / ANGLE_DIVS;
    double DIST_DIVS = 4;//;//4;
    double DIST_STEP = 0.9;//1.1;
    ...
    for ( int d = 1; d <= DIST_DIVS; ++d )
    {
        ...
        for ( int a = 0; a < ANGLE_DIVS; a += a_step )
        {
            ...
            int move_dist_penalty_step = 0;
            {
                ...
                move_dist_penalty_step = static_cast< int >( std::floor( player_line_dist * 0.2 ) );
            }
            ...
            if ( receive_point.x > SP.pitchHalfLength() - 2.0
                 || receive_point.x < -SP.pitchHalfLength() + 4.0
                 || receive_point.absY() > SP.pitchHalfWidth() - 2.0 )
            {
#ifdef DEBUG_LEADING_PASS
                dlog.addText( Logger::PASS,
            ...
            if(logtest)cout<<"   rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"<<endl;
            const int receiver_step = predictReceiverReachStep( wm,receiver,
                                                                receive_point,
                                                                true )
                + move_dist_penalty_step;
            if(logtest)cout<<"   rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"<<endl;
            if(logtest)cout<<"   ! tm recive in"<<receiver_step<<endl;
            const AngleDeg ball_move_angle = ( receive_point - M_first_point ).th();
...
#endif
            if(logtest)cout<<"  #target : "<<receive_point<<endl;
            createPassCommon( wm,
                              receiver, receive_point,
                              start_step, max_step,
                              min_ball_speed, max_ball_speed,
                                             ...
                                             const Receiver & receiver )
{
	...
	int ANGLE_DIVS = 14;
	double MIN_ANGLE = -40.0;
	double MAX_ANGLE = +40.0;
	double ANGLE_STEP = ( MAX_ANGLE - MIN_ANGLE ) / ANGLE_DIVS;

	double MIN_MOVE_DIST = 6.0;
	double MAX_MOVE_DIST = 30.0 + 0.001;
	double MOVE_DIST_STEP = 2.0;
    ...
    const double min_receive_x = std::min( std::min( std::max( 0.0, M_first_point.x ),
    ...
    for ( int a = 0; a <= ANGLE_DIVS; ++a )
    {
              ...
              move_dist += MOVE_DIST_STEP )
        {
            ...
            int number_of_candid = 0;
            if(start_step-receiver_step>1){
            	number_of_candid=1;
            }else{
            	number_of_candid=2;
            }
                                            ...
                                            const char * description)
{
    ...
    for ( int step = min_step; step <= max_step; ++step )
    {
        ...
        int o_step = predictOpponentsReachStep( wm,
                                                ...
                                                step + ( kick_count - 1 ) + 5,
                                                &opponent,
                                                (kick_bonus?kick_count - 1:0),
                                                opp_min_cycle_dif);

        if(wm.opponentsFromBall().size()>0)
        if(wm.opponentsFromBall().front()->distFromBall()<2 && kick_count>1)
        	pass_safe+=4;

        if(opp_min_cycle_dif==2){
        	pass_safe += 2;
        }else if(opp_min_cycle_dif ==1){
        	pass_safe += 4;
        }
        ...
        {
            if ( o_step <= step + (kick_bonus?0:( kick_count - 1 )) )
            {
                failed = true;
            }
        }
        ...
        if ( failed )
        {
            ...
            dlog.addText( Logger::PASS,
                          ...
                          kick_count );
#endif
            if(logtest)cout<<"   intercept this pass"<<endl;
            break;
        }
        if(logtest)cout<<"   this pass is ok"<<endl;
 ...
 */
int
StrictCheckPassGenerator::predictReceiverReachStep( const WorldModel & wm,
													const Receiver & receiver,
                                                    const Vector2D & pos,
                                                     ...
                                                     int & opp_min_dif_cycle)
{
    ...
    for ( OpponentCont::const_iterator o = M_opponents.begin();
          ...
          ++o )
    {
    	if(logtest)cout<<"    $opp : "<<o->player_->unum()<<endl;
                                                    ...
                                                    int & opp_min_dif_cycle)
{
    ...
    min_cycle -= opponent.player_->posCount();
...
min_cycle-=2;
    for ( int cycle = std::max( 1, min_cycle ); cycle <= max_cycle; ++cycle )
    {
        ...
        if ( dash_dist > ptype->realSpeedMax()
             * ( cycle + std::min( opponent.player_->posCount(), 8 ) ) )
        {
            continue;
        }
        ...
        if(logtest)cout<<"      c "<<cycle<<" ball : "<<ball_pos<<" ndash : "<<n_dash<<endl;
        if ( n_dash > cycle + opponent.player_->posCount() )
        {
            if(logtest)cout<<"        not intercept"<<endl;
            continue;
        }
		...
		if( cycle - (n_step - bonus_step)<opp_min_dif_cycle ){
			opp_min_dif_cycle = cycle - (n_step - bonus_step);
		}
        if ( n_step - bonus_step <= cycle )
        {
            if(logtest)cout<<"        intercept"<<endl;
            return cycle;
        }
        if(logtest)cout<<"        not intercept"<<endl;
    }
