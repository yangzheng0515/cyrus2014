...
void debug_paint_failed(const int count, const Vector2D & receive_point) {
	dlog.addRect(Logger::DRIBBLE, receive_point.x - 0.1, receive_point.y - 0.1,
			0.2, 0.2, "#ff0000");
	char num[8];
 ...
 */
void SelfPassGenerator::generate(const WorldModel & wm) {
 ...
 */
void SelfPassGenerator::createCourses(const WorldModel & wm) {
	...
	const int max_dash =
			(ball_pos.x < -20.0 ? 6 : ball_pos.x < -5.0 ? 7 :
				ball_pos.x < 10.0 ? 17 : ball_pos.x < 20.0 ? 22 : 22);

	const PlayerType & ptype = wm.self().playerType();

	const double max_effective_turn = ptype.effectiveTurn(SP.maxMoment(),
	...
	for (int a = 0; a < ANGLE_DIVS; ++a) {
		...
		for (; n_dash >= min_dash; n_dash -= dash_dec) {
			...
			CooperativeAction::Ptr ptr(
					new Dribble(wm.self().unum(), receive_pos, first_speed, 1, // 1 kick
							n_turn, n_dash, "SelfPass"));
 ...
 */
void SelfPassGenerator::createSelfCache(const WorldModel & wm,
	...
	const double stamina_thr = (
			wm.self().staminaModel().capacityIsEmpty() ? -ptype.extraStamina() // minus value to set available stamina
	...
	for (int i = 0; i < n_dash; ++i) {
		...
		double available_stamina = std::max(0.0,
				stamina_model.stamina() - stamina_thr);
		double actual_dash_power = std::min(dash_power, available_stamina);
		double accel_mag = actual_dash_power * ptype.dashPowerRate()
 ...
 */
bool SelfPassGenerator::canKick(const WorldModel & wm, const int n_turn,
		const int n_dash, const Vector2D & receive_pos) {
	const ServerParam & SP = ServerParam::i();
	...
	const Vector2D ball_next = ball_pos
			+ (receive_pos - ball_pos).setLengthVector(first_speed);

	if (my_next.dist2(ball_next)
			< std::pow(
					wm.self().playerType().playerSize() + SP.ballSize() + 0.1,
					2)) {
#ifdef DEBUG_PRINT_FAILED_COURSE
		dlog.addText( Logger::DRIBBLE,
	...
	for (PlayerPtrCont::const_iterator o = wm.opponentsFromSelf().begin();
		...
		const double control_area = (
				((*o)->goalie() && ball_next.x > SP.theirPenaltyAreaLineX()
		...
		if ((*o)->bodyCount() <= 1) {
			o_next += Vector2D::from_polar(
					SP.maxDashPower() * ptype->dashPowerRate()
							* ptype->effortMax(), (*o)->body());
		} else {
			o_next += (*o)->vel().setLengthVector(
					SP.maxDashPower() * ptype->dashPowerRate()
							* ptype->effortMax());
		}
 ...
 */
bool SelfPassGenerator::checkOpponent(const WorldModel & wm, const int n_turn,
		const int n_dash, const Vector2D & ball_pos,
		const Vector2D & receive_pos) {
	const ServerParam & SP = ServerParam::i();

	const int self_step = (kick_bonus_self ? 0 : 1) + n_turn + n_dash;
	...
	for (PlayerPtrCont::const_iterator o = wm.opponentsFromSelf().begin();
		...
		const Vector2D & opos = (
				(*o)->seenPosCount() <= (*o)->posCount() ?
						(*o)->seenPos() : (*o)->pos());
		const Vector2D ball_to_opp_rel = (opos - ball_pos).rotatedVector(
				-target_angle);
...
//				is_pass, 1.2, (kick_bonus_self ? 1 : 0)); //cyclesToReachDistance( dash_dist );
		...
		if (!is_pass) {
			...
			double dif = ((opp_pos - wm.ball().pos()).th()
					- (wm.ball().pos() - ball_pos).th()).abs();
			if (dif > 180)
				dif = 360 - dif;
			if (dif > 120) {
				if (ball_to_opp_rel.x > 0.8) {
					bonus_step += bound(0, (*o)->posCount() - 1, 2);
				} else {
					int penalty_step = (
							(receive_pos.x > wm.offsideLineX()
									|| receive_pos.x > 35.0) ? 1 : 0);
					bonus_step = bound(0, (*o)->posCount() - penalty_step, 1);
				}
