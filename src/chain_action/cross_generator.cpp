                             ...
                             const AbstractPlayerObject * receiver )
{
    ...
    // angle loop
    //
    for ( int a = (wm.self().isKickable()?-2:0)-2; a < (wm.self().isKickable()?3:1); ++a )
    {
        ...
        // distance loop
        //
        for ( int d = 0; d < (wm.self().isKickable()?5:1); ++d )
        {
                  ...
                  ++step )
            {
                ...
                if ( ! checkOpponent( M_first_point,
                                      ...
                                      first_ball_speed,
                                      cross_angle,
                                      step + kick_count - 1,wm ) ) // 1 step penalty for observation delay
                {
                               ...
                               const WorldModel & wm)
{
          ...
          ++o )
    {
        ...
        int min_cycle = std::max(0,FieldAnalyzer::estimate_min_reach_cycle( opponent_pos,
                                                                       ptype->realSpeedMax(),
                                                                       first_ball_pos,
                                                                       ball_move_angle )-1);
